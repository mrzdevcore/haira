...
export class HairaField extends HTMLElement {
  ...
  private renderInput(name: string, type: string): string {
    switch (type) {
      case "bool":
        return `
          <div class="toggle-row">
            <span class="toggle-label">${this.esc(name)}</span>
            <label class="toggle">
      ...
      case "file":
        return `
          <label class="field-label">${this.esc(name)}</label>
      ...
      case "int":
        return `
          <label class="field-label" for="f-${name}">${this.esc(name)} <span class="field-sublabel">integer</span></label>
          <input type="number" id="f-${name}" name="${name}" step="1" placeholder="0">`;
      case "float":
        return `
          <label class="field-label" for="f-${name}">${this.esc(name)} <span class="field-sublabel">number</span></label>
          <input type="number" id="f-${name}" name="${name}" step="any" placeholder="0.0">`;
      default:
        return `
          <label class="field-label" for="f-${name}">${this.esc(name)}</label>
  ...
  private setupFileDrop(shadow: ShadowRoot) {
    ...
    const setFile = (file: File) => {
      ...
      textEl.innerHTML = `<span class="filename">${this.esc(file.name)}</span><span class="filesize">${size}</span>`;
    };
    ...
    zone.addEventListener("drop", (e: DragEvent) => {
      ...
      if (file) {
        setFile(file);
        // Also update the input for FormData
