...
\code{continue} & \code{defer} & \code{else} & \code{enum} & \code{export} \\
\code{false} & \code{fn} & \code{for} & \code{from} & \code{if} \\
\code{impl} & \code{import} & \code{in} & \code{match} & \code{nil} \\
\code{not} & \code{or} & \code{pub} & \code{return} & \code{select} \\
\code{spawn} & \code{struct} & \code{trait} & \code{true} & \code{try} \\
\code{type} & \code{while} & & & \\
\midrule
...
keyword = "agent" | "and" | "assert" | "async" | "break"
        | "catch" | "const" | "continue" | "default" | "defer"
        | "else" | "enum" | "err" | "errdefer" | "export"
        | "false" | "fn" | "for" | "from" | "if" | "impl"
        | "import" | "in" | "let" | "match" | "nil" | "none"
        | "not" | "ok" | "oncancel" | "onerror" | "onsuccess"
        | "or" | "orelse" | "provider" | "pub" | "return"
        | "select" | "some" | "spawn" | "step" | "struct"
        | "test" | "tool" | "trait" | "true" | "try" | "type"
        | "while" | "workflow" ;
