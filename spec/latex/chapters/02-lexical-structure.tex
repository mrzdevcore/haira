...
\begin{tabular}{lllll}
\toprule
\code{ai} & \code{and} & \code{as} & \code{break} & \code{catch} \\
\code{chan} & \code{continue} & \code{defer} & \code{else} & \code{enum} \\
\code{false} & \code{fn} & \code{for} & \code{from} & \code{if} \\
\code{import} & \code{in} & \code{match} & \code{nil} & \code{not} \\
\code{or} & \code{return} & \code{select} & \code{spawn} & \code{struct} \\
\code{true} & \code{try} & \code{type} & \code{while} & \\
\bottomrule
\end{tabular}
...
\begin{note}
The keywords \keyword{ai}, \keyword{catch}, \keyword{defer}, \keyword{select}, \keyword{spawn}, and \keyword{try} are reserved for future language features. Using them will produce a clear error message.
\end{note}
...
keyword = "ai" | "and" | "as" | "break" | "catch" | "chan"
        | "continue" | "defer" | "else" | "enum" | "false"
        | "fn" | "for" | "from" | "if" | "import" | "in"
        | "match" | "nil" | "not" | "or" | "return" | "select"
        | "spawn" | "struct" | "true" | "try" | "type" | "while" ;
