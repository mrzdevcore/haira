...
The \type{stream<T>} type represents a lazy, asynchronous sequence of values produced over time. Streams are the primary mechanism for handling token-by-token output from agents and for streaming responses over WebSocket connections.

\begin{lstlisting}
// Stream of string tokens from an agent
for chunk in Assistant.stream("Tell me a story") {
    io.print(chunk)
}
\end{lstlisting}

\subsection{Stream Type Syntax}
...
\begin{lstlisting}
// Common stream types
stream<string>    // Stream of text tokens (agent output)
stream<byte>      // Stream of raw bytes
stream<Event>     // Stream of typed events
\end{lstlisting}

\subsection{Consuming Streams}

Streams are consumed with \keyword{for} loops. Each iteration yields the next value as it becomes available:

\begin{lstlisting}
// Iterate over a stream
tokens: stream<string> = Researcher.stream("Explain quantum computing")
...
\subsection{Streams as Return Types}
...
When a workflow returns \type{stream<T>}, the runtime streams each element to the client as it is produced---no buffering.

\subsection{Stream Properties}

\begin{itemize}
    \item \textbf{Lazy} -- Values are produced on demand, not computed upfront
    \item \textbf{Single-use} -- A stream can only be iterated once
    \item \textbf{Asynchronous} -- The \keyword{for} loop suspends until the next value is available
    \item \textbf{Finite} -- Streams eventually terminate (the loop exits when the source is exhausted)
\end{itemize}

\begin{note}
Streams are distinct from channels (\type{chan<T>}). A channel is a bidirectional communication primitive between concurrent tasks. A stream is a unidirectional, read-only sequence---typically produced by an agent or an external data source.
\end{note}
