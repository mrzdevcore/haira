...
fn (r: Rectangle) scale(factor: float) -> Rectangle {
    return Rectangle{
...
\subsection{Method Receiver}

The receiver can be by value or by reference:

\begin{lstlisting}
// By value (receiver is copied)
fn (p: Point) distance_to(other: Point) -> float {
    dx = p.x - other.x
    dy = p.y - other.y
    return math.sqrt(dx*dx + dy*dy)
}

// By mutable reference (can modify receiver)
fn (p: *Point) translate(dx: float, dy: float) {
    p.x += dx
    p.y += dy
}
\end{lstlisting}
