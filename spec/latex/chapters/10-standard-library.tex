...
encoding.base64_decode(string)

// Hex
encoding.hex_encode(data)
encoding.hex_decode(string)
\end{lstlisting}

\section{Agentic Modules}
...
// Returns a map with name, system, temperature, max_steps
config = agents.code_reviewer()
config = agents.planner()
config = agents.security_reviewer()
config = agents.summarizer()
config = agents.data_analyst()
config = agents.customer_support()
config = agents.tdd_guide()
config = agents.doc_writer()

// Use with create_agent() for dynamic agent creation
reviewer = create_agent(config, my_provider, [my_tool])
\end{lstlisting}

\subsection{observe Module}
...
// Track costs
total = observe.cost()
agent_cost = observe.agent_cost("Writer")
usage = observe.usage()
...
observe.start(server)  // Serves /_observe
\end{lstlisting}

\section{Testing}

\haira{} has built-in testing support. Tests are declared with the \keyword{test} keyword followed by a name and a block of assertions.
...
The \keyword{assert} statement checks a condition. If it fails, the test reports the failure and continues.

\begin{lstlisting}
// Boolean assertion
assert is_valid(input)

// Equality check (reports "got X, want Y" on failure)
assert result == 42
...
The compiler generates Go test functions and runs them with \code{go test}. Each \keyword{test} block becomes a subtest. Output follows standard test format:
