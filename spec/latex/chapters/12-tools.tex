...
Tools support optional \code{@before} and \code{@after} hooks for pre/post-processing logic. Hooks are useful for validation, logging, rate limiting, or cleanup.

\subsection{@before Hook}

The \code{@before} block executes before the tool body:

\begin{lstlisting}
tool create_user(name: string, email: string) -> User {
    ...
    user = db.insert("users", { name: name, email: email })
    return user
}
\end{lstlisting}

\subsection{@after Hook}

The \code{@after} block executes after the tool body completes:
...
\begin{note}
Hooks are optional. \code{@before} and \code{@after} must appear after the triple-quoted description. If a \code{@before} hook fails (e.g., via \code{assert}), the tool body does not execute.
\end{note}
