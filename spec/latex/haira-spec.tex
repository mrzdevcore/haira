...
\lstdefinelanguage{Haira}{
    keywords={fn, struct, enum, type, if, else, for, while, match, return, break, continue, import, export, from, as, nil, true, false, in, and, or, not, spawn, chan, select, try, catch, defer, pub, trait, impl, agent, provider, tool, workflow},
...
agent     and       as        break     catch
chan      const     continue  defer     else
enum      export    false     fn        for
from      if        impl      import    in
match     nil       not       or        provider
pub       return    select    spawn     struct
tool      trait     true      try       type
unsafe    where     while     workflow
\end{lstlisting}
...
\code{encoding} & Base64/hex encoding \\
\midrule
\multicolumn{2}{l}{\textbf{Tool Packs} (importable tools for agents)} \\
\midrule
\code{tools/http} & HTTP tools (get, post, put, delete) \\
\code{tools/slack} & Slack integration (send, read) \\
\code{tools/github} & GitHub API (create\_issue, list\_prs) \\
\code{tools/postgres} & PostgreSQL (query, insert) \\
\code{tools/email} & Email sending \\
\code{tools/fs} & File system tools for agents \\
\bottomrule
\end{longtable}
