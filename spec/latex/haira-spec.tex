...
\begin{enumerate}
    ...
    \item \textbf{Native performance} -- Compiles to standalone native binaries via Cranelift.
\end{enumerate}
...
15 (lowest) & \code{=} \code{+=} \code{-=} \code{*=} \code{/=} \code{\&=} \code{|=} \code{\^{}=} \code{<<=} \code{>>=} \code{>>>=} & Right \\
\bottomrule
\end{longtable}
