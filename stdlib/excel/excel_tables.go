...
func PostgresGenerateUpsertWithConflicts(tables *ExcelTables, schema *postgres.PgSchema, conflicts map[string]any) *postgres.SqlResult {
	...
	for _, name := range orderedNames {
		...
		// that exists in the DB schema. AutoGenerated (serial) columns are included
		// when the Excel explicitly provides values for them — the user knows what
		// they're doing (e.g. upserting by an integer PK). Columns absent from the
		// Excel headers are still excluded.
